...
[[ -n "$PUP_CUSTOM_DIR" ]] && BASE="$PUP_CUSTOM_DIR"
[[ -z "$BASE" ]] && die "Unable to determine install base. Try setting PUP_CUSTOM_DIR env var."
...
find "$BASE/r10k/postrun_parts/" -type f -name '*.py' -print \
| while read ; do
    set_shebang_path "$V_PYTHON" "$REPLY"
done

# r10k.yaml config is managed by Puppet; do not attempt changes from here
