...
def get_base():
    if not 'BASE' in resources:
        ...
        resources['BASE'] = pathlib.Path( 
            os.getenv( 'PUP_CUSTOM_DIR', 
                default='/etc/puppetlabs/local' ) )
...
def get_args():
    if 'args' not in resources:
        ...
        for k in columns:
            long_opt = '--' + k
            short_opt = '-' + k[0].upper()
            cols_group.add_argument( long_opt, short_opt )
    
        # Actions
        action_group = parser.add_argument_group( 'Action', 
            description='If no action given, defaults to --list.' )
        ...
        action_group.add_argument( '--production', '--prod', dest='action',
            ...
            help='''Shortcut for "--chnode --environment production"
                 ''')
        action_group.add_argument( '--test', dest='action',
            action='store_const', const='test',
            help='''Shortcut for "--chnode --environment test"
                 ''')
